...
A lightweight batch worker that ingests customer feedback from a database, performs NLP analysis (sentiment + zero‑shot topic classification), computes word counts, and optionally indexes results into OpenSearch for search/analytics. The worker maintains job state in the database and runs on a simple schedule.
...
4. Fetch next batch of feedbacks where `id > last_processed_id` (default `batch_size=50`).
...
8. Loop: scheduled every 10 seconds.

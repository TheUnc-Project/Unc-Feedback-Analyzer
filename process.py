...
def process_partition_static(
    ...
    for idx, text in enumerate(texts):
        ...
        try:
            ...
            top_topics = sorted(filtered_topics, key=lambda x: x[1], reverse=True)[:3]
...
class FeedbackWorker:
    def __init__(self):
        ...
        self.cluster = LocalCluster(n_workers=1, threads_per_worker=1, processes=True)
    ...
    def get_next_feedbacks(self, last_processed_id=0, config=None):
        ...
        batch_size = config.config.get("batch_size", 50)
        query = (
            select(Feedback)
    ...
    def run_job(self):
        ...
        if not config:
            config = JobConfig(config={"batch_size": 50, "run_next_job": True})
            self.db.add(config)
            self.db.commit()
...
def main():
    ...
    try:
        schedule.every(10).seconds.do(worker.run_job)
        logger.info("Worker started. Scheduled job every 10 seconds.")
        while True:
